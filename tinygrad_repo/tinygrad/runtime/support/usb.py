...
class CustomASM24Controller:
  ...
  def __init__(self, usb:USB3):
    ...
    # The firmware power-on sequence retrains PCIe. Give the bridge time to
    # report link-up before treating an enumerated USB device as unusable.
    ...
    if ltssm != self.PCIE_LINK_READY:
      ...
      raise RuntimeError(f"USB bridge reset requested after PCIe link retrain failed (LTSSM=0x{ltssm:02X})")

  def set_pcie_power(self, enabled:bool, timeout:int=10000): self.usb.control_write(0xF3, value=int(enabled), timeout=timeout)

  def reset_usb_bridge(self): self.usb.reset()

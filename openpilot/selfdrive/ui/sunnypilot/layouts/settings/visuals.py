...
from openpilot.sunnypilot.modeld_v2.traffic_signal_settings import (
  is_enabled as traffic_signal_yolo_enabled,
  set_enabled as set_traffic_signal_yolo_enabled,
)
...
FACTORY_CLUSTER_STATUS_TEXT = {
  ...
  FactoryClusterStatus.UNAVAILABLE_UNVERIFIED_PROFILE.value: (
    "Unavailable: vehicle firmware and CAN profile not verified",
    "無法使用：車輛韌體與 CAN 設定檔尚未驗證",
  ),
  FactoryClusterStatus.UNAVAILABLE_NO_STOCK_TEMPLATE.value: (
    "Unavailable: original target message not captured",
    "無法使用：尚未擷取原廠目標訊息",
  ),
  FactoryClusterStatus.UNAVAILABLE_INVALID_STOCK_TEMPLATE.value: (
    "Unavailable: original target message failed validation",
    "無法使用：原廠目標訊息驗證失敗",
  ),
  FactoryClusterStatus.UNAVAILABLE_STALE_STOCK_MESSAGE.value: (
    "Unavailable: original target message is stale",
    "無法使用：原廠目標訊息已過期",
  ),
  FactoryClusterStatus.UNAVAILABLE_RADAR_DATA.value: (
    "Unavailable: real target data is stale or missing",
    "無法使用：實際目標資料過期或缺失",
  ),
  FactoryClusterStatus.UNAVAILABLE_TARGET_ENCODING.value: (
    "Unavailable: real target cannot be encoded safely",
    "無法使用：實際目標無法安全編碼",
  ),
  FactoryClusterStatus.UNAVAILABLE_UNSUPPORTED_MESSAGE.value: (
    "Unavailable: message is not authorized by Panda safety",
    "無法使用：Panda safety 未允許此訊息",
  ),
  FactoryClusterStatus.BLOCKED_SENDER_CONFLICT.value: (
    "Blocked: another ECU is sending the same message",
    "已阻擋：其他 ECU 正在傳送相同訊息",
  ),
  FactoryClusterStatus.ACTIVE_STOCK_PASSTHROUGH.value: (
    "Active: preserving original vehicle targets",
    "啟用：保留原廠車輛目標",
  ),
  FactoryClusterStatus.ACTIVE_REAL_TARGETS.value: (
    "Active: displaying verified real targets",
    "啟用：顯示已驗證的實際目標",
  ),
  FactoryClusterStatus.ACTIVE_BSM_REGION.value: (
    "Active: blind-spot region warning only",
    "啟用：僅顯示盲點區域警告",
  ),
...
FACTORY_CLUSTER_DESCRIPTION = (
  ...
  "在 sunnypilot 縱向控制或實驗模式啟用時，保留 Kia 原廠儀表左右側實際車輛圖示。"
  + "此功能僅供顯示，不會啟用 Kia 自動換道，也不會將側向目標用於駕駛決策。"
  + "若韌體不支援、資料過期或 CAN 傳送端衝突，系統不會額外產生車輛圖示。變更此設定後需要重新啟動。",
)

ADJACENT_MARKERS_DESCRIPTION = (
  "Show yellow markers for high-confidence model lead candidates outside the current driving path. "
  + "The driving model does not provide a complete object list, so some nearby vehicles may not appear.",
  "在目前行駛路徑外，以黃色標記顯示模型高可信度的前車候選目標。"
  + "駕駛模型並未提供完整物件清單，因此部分鄰近車輛可能不會顯示。",
)

PREDICTED_STOP_DESCRIPTION = (
  "Draw a line at the model-predicted stopping position. The marker is latched through brief prediction dropouts "
  + "and disappears after the vehicle has stopped and starts moving again. Display only; it does not affect braking or longitudinal control.",
  "在畫面上以線條標示模型預計的停止位置。模型預測短暫消失時仍會保留標記，"
  + "車輛完成停止並再次起步後才會清除。此功能僅供顯示，不會影響煞車或縱向控制。",
)

CORNER_RADAR_DESCRIPTION = (
  "Show yellow radar candidates beside the speedometer, separately from blind-spot icons. RADAR* means experimental: "
  + "sensor mounting and validity still need vehicle validation. Display only; no control decisions.",
  "在時速表兩側以黃色標記顯示角雷達候選目標，與盲點圖示分開顯示。RADAR* 代表實驗功能；"
  + "感測器安裝位置與資料有效性仍需實車驗證。僅供顯示，不參與控制決策。",
)

GREEN_LIGHT_DESCRIPTION = (
  "A chime and on-screen alert will play when the traffic light you are waiting for turns green and you have no vehicle in front of you."
  + "<br>Note: This chime is only designed as a notification. It is the driver's responsibility to observe their environment and make decisions accordingly.",
  "等待中的號誌轉為綠燈且前方沒有車輛時，播放提示音並顯示畫面提醒。"
  + "<br>注意：此提示僅供通知，駕駛仍須自行觀察環境並做出安全判斷。",
)

LEAD_DEPART_DESCRIPTION = (
  "A chime and on-screen alert will play when you are stopped, and the vehicle in front of you starts moving."
  + "<br>Note: This chime is only designed as a notification. It is the driver's responsibility to observe their environment and make decisions accordingly.",
  "本車停止且前方車輛開始移動時，播放提示音並顯示畫面提醒。"
  + "<br>注意：此提示僅供通知，駕駛仍須自行觀察環境並做出安全判斷。",
)

YOLO_DESCRIPTION = (
  "Display-only traffic signal recognition using the UsamaMasood12 YOLO11s model. When Chestnut is active, detection runs only in spare GPU "
  + "headroom after the driving model. It automatically backs off when model latency or frame drops increase. Results are never used for steering, "
  + "acceleration, braking, or planning.",
  "使用 UsamaMasood12 YOLO11s 模型進行僅供顯示的交通號誌辨識。Chestnut 啟用時，只會在駕駛大模型完成後利用剩餘 GPU 餘裕執行；"
  + "當模型延遲或掉幀增加時會自動退避。辨識結果不會用於轉向、加速、煞車或路徑規劃。",
)

RAINBOW_STYLE_DESCRIPTION = (
  "Blue bars animate with actual vehicle acceleration and deceleration. A model-predicted stop is shown for any cause, including traffic lights or "
  + "stop signs. The model does not identify light colors or sign types.",
  "藍色動態條會依車輛實際加速與減速變化。模型預測需要停止時會顯示停止提示，不論原因可能是號誌、停止標誌或其他情況；"
  + "模型本身不會辨識號誌顏色或標誌類型。",
)
...
class VisualsLayout(Widget):
  ...
  def _initialize_items(self):
    self._toggle_defs = {
      "AdjacentLaneObjectMarkers": (
        ...
        lambda: vtr(*ADJACENT_MARKERS_DESCRIPTION),
        None,
      ),
      "BlindSpot": (
        lambda: vtr("Show Blind Spot Warnings", "顯示盲點警告"),
        lambda: vtr(
          "Display a warning when the vehicle reports an object in the left or right blind spot. Requires vehicle BSM support.",
      ...
      "RainbowMode": (
        ...
        lambda: vtr(
          "Choose the original rainbow path or dynamic blue acceleration and deceleration bars below. Display only.",
      ...
      "RoadNameToggle": (
        ...
        lambda: vtr(
          "Displays the name of the road the car is traveling on.<br>The OpenStreetMap database of the location must be downloaded from the OSM "
          + "panel to fetch the road name.",
      ...
      "GreenLightAlert": (
        ...
        lambda: vtr(*GREEN_LIGHT_DESCRIPTION),
        None,
      ),
      "LeadDepartAlert": (
        lambda: vtr("Lead Departure Alert (Beta)", "前車起步提醒（Beta）"),
        lambda: vtr(*LEAD_DEPART_DESCRIPTION),
        None,
      ),
      "TrueVEgoUI": (
    ...
    self._traffic_signal_yolo = toggle_item_sp(
      ...
      description=lambda: vtr(*YOLO_DESCRIPTION),
    ...
    self._blindspot_style = multiple_button_item_sp(
      ...
      description=lambda: vtr(
        "Choose the original small icons beside the speedometer or flashing light bars along the left/right screen edges.",
    ...
    self._dev_ui_info = multiple_button_item_sp(
      ...
      buttons=[
        lambda: vtr("Off", "關閉"),
        lambda: vtr("Bottom", "下方"),
        lambda: vtr("Right", "右側"),
        lambda: vtr("Right & Bottom", "右側與下方"),
      ],
      param="DevUIInfo",
      button_width=350,
      inline=False,
    ...
    self._rainbow_style = multiple_button_item_sp(
      ...
      description=lambda: vtr(*RAINBOW_STYLE_DESCRIPTION),
  ...
  def _on_traffic_signal_yolo(self, state=None):
    # ToggleAction invokes callbacks with state. Ignore ListItem's later no-argument notification.
  ...
  def _update_state(self):
    ...
    chestnut_available = bool(ui_state.chestnut_present or ui_state.chestnut_active)
    self._traffic_signal_yolo.set_visible(chestnut_available and ui_state.model_runner_tinygrad)
    ...
    hkg_canfd = (
      ...
      and ui_state.CP.brand == "hyundai"
      and bool(ui_state.CP.flags & HyundaiFlags.CANFD)
    )
    ...
    factory_cluster_toggle.set_description(f"<b>{status_text}</b><br><br>{vtr(*FACTORY_CLUSTER_DESCRIPTION)}")

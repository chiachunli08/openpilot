...
import requests
from requests.exceptions import ConnectionError as RequestsConnectionError, SSLError, RequestException, HTTPError
from openpilot.common.params import Params
from openpilot.common.swaglog import cloudlog
from openpilot.selfdrive.modeld.model_manager.helpers import REQUIRED_TINYGRAD_REF, is_bundle_version_compatible
...
class ModelParser:
  ...
  def _safe_name(cls, value: object) -> str:
    if not isinstance(value, str) or not value or not cls.SAFE_NAME.fullmatch(value):
      raise ValueError(f"invalid model artifact name: {value!r}")
    return value

  @classmethod
  def _safe_sha256(cls, value: object) -> str:
    if not isinstance(value, str) or not cls.SAFE_SHA256.fullmatch(value):
      raise ValueError("invalid model artifact sha256")
    return value.lower()
  ...
  def _parse_download_uri(download_uri_data) -> custom.ModelManagerSP.DownloadUri:
    ...
    download_uri.sha256 = ModelParser._safe_sha256(download_uri_data.get("sha256"))
    return download_uri

  @staticmethod
  def _parse_chunk(chunk_data) -> custom.ModelManagerSP.Chunk:
    chunk = custom.ModelManagerSP.Chunk()
    chunk.fileName = ModelParser._safe_name(chunk_data.get("file_name"))
    chunk.sha256 = ModelParser._safe_sha256(chunk_data.get("sha256"))
    return chunk

  @staticmethod
  def _parse_artifact(artifact_data) -> custom.ModelManagerSP.Artifact:
    artifact = custom.ModelManagerSP.Artifact()
    artifact.fileName = ModelParser._safe_name(artifact_data.get("file_name"))
  ...
  def _parse_bundle(bundle) -> custom.ModelManagerSP.ModelBundle:
    ...
    if model_bundle.runner != custom.ModelManagerSP.Runner.tinygrad:
      raise ValueError(f"unsupported Carrot model runner: {model_bundle.runner}")
    ...
    model_bundle.ref = ModelParser._safe_name(bundle.get("ref"))

    return model_bundle

  @staticmethod
  def parse_models(json_data: dict) -> list[custom.ModelManagerSP.ModelBundle]:
    if not isinstance(json_data, dict) or not isinstance(json_data.get("bundles", []), list):
      raise ValueError("invalid model catalog")
    if json_data.get("tinygrad_ref") != REQUIRED_TINYGRAD_REF:
      raise ValueError("model catalog requires an unsupported tinygrad revision")
    found_bundles = []
    for bundle in json_data.get("bundles", []):
      try:
        parsed = ModelParser._parse_bundle(bundle)
      except (KeyError, TypeError, ValueError) as e:
        cloudlog.warning(f"Skipping invalid model bundle: {e}")
        continue
      if is_bundle_version_compatible(parsed.to_dict()):
        found_bundles.append(parsed)

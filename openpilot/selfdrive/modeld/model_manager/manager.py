...
from openpilot.selfdrive.modeld.model_manager.fetcher import ModelFetcher
from openpilot.selfdrive.modeld.model_manager.helpers import (ACTIVE_BUNDLE_KEYS, get_active_bundle, get_selected_bundle,
...
class ModelManagerSP:
  ...
  def _report_status(self) -> None:
    ...
    model_manager_state.availableBundlesQcom = self.source_models.get("qcom", [])
    model_manager_state.availableBundlesUsbGpu = self.source_models.get("usbgpu", [])
    if active_qcom := get_selected_bundle(self.params, "qcom"):
      model_manager_state.activeBundleQcom = active_qcom
    if active_usbgpu := get_selected_bundle(self.params, "usbgpu"):
      model_manager_state.activeBundleUsbGpu = active_usbgpu
    model_manager_state.selectedSource = self.selected_source
  ...
  async def _download_bundle(self, model_bundle: custom.ModelManagerSP.ModelBundle, destination_path: str, source: str) -> None:
    ...
    try:
      ...
      self.params.put(ACTIVE_BUNDLE_KEYS[source], model_bundle.to_dict())
  ...
  def main_thread(self) -> None:
    ...
    while True:
      try:
        self.chestnut_present = usbgpu_present()
        self.params.put_bool("UsbGpuPresent", self.chestnut_present)

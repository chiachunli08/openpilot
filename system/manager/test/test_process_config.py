...
class DriverMonitoringParams:
  """Params stand-in for the driver_monitoring predicate. Driver monitoring
  is hard-disabled on this fork, so the predicate always returns False
  regardless of IsDriverViewEnabled."""
  def __init__(self, *, driver_view: bool = False):
    self.values = {"IsDriverViewEnabled": driver_view}
...
def test_driver_monitoring_predicate_always_false(started, driver_view):
  # Driver monitoring is hard-disabled: predicate returns False regardless of
  # whether the car is started or in driverview.
  params = DriverMonitoringParams(driver_view=driver_view)
  assert driver_monitoring(started, params, car.CarParams.new_message(), SimpleNamespace()) is False


def test_dmonitoring_processes_are_disabled():
  # Both DM processes must be permanently disabled (enabled=False) on this fork.
  assert not managed_processes["dmonitoringmodeld"].enabled
  assert not managed_processes["dmonitoringd"].enabled
  # And gated by the driver_monitoring predicate, not the legacy driverview predicate.
